      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex flex-col items-center text-center mb-12 space-y-4">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium max-w-lg">
            {dict?.stats?.subtitle || "Tizimdagi barcha terminlar, qisqartmalar va tillar bo'yicha umumiy statistika."}
          </p>
        </div>

        {/* Statistika Kartochkalari Komponenti (Client Component) */}
        ...
        <div className="mt-12 p-8 rounded-[3rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm text-center">
          <p className="text-slate-400 dark:text-slate-500 text-sm font-bold uppercase tracking-widest">
            {dict.stats.last_update || "Oxirgi yangilanish"}: {new Date().toISOString().split('T')[0]}
          </p>
        </div>
      </div>
    </div>
